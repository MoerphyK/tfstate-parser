...
def upload_report_to_s3(file, timestamp):
        '''
        Upload the report to S3
        params: file - Path to the report file
                timestamp - Timestamp for the report
        '''
        # Create the S3 client
        s3_client = boto3.client('s3')
        # Upload the report to S3
        s3_client.upload_file(file, reporting_bucket, f'{timestamp}/report_{timestamp}.pdf')
